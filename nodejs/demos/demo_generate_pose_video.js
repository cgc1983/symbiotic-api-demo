...
const { GeneratePoseVideo } = require("../models/models");
const config = require("../config/config");

async function main() {
  try {
    logger.info("Starting generate-pose-video process");
    ...
    const requestModel = new GeneratePoseVideo(
      characterImageName,
      "1", // 1:Disney; 2:Pixel; 3:Anime; 4:Ghibli;
      "A beautiful character in animated pose",
    ...
    const response = await makeAuthenticatedRequest(
      "POST",
      "/api/v1/generate-pose-video",
      requestModel
    );

    if (response.status !== 200) {
      logger.error(
        `Generate-pose-video execution failed, status code: ${response.status}`
      );
      return;
    }
    ...
    if (promptModel.code !== 0) {
      logger.error(
        `Generate-pose-video execution failed, error message: ${promptModel.msg}`
      );
      return;
    }
    ...
    const history = await getTaskHistory("7"); // Assuming tool_id for generate-pose-video is 7
    ...
    logger.info("Generate pose video demo completed successfully!");
  } catch (error) {
