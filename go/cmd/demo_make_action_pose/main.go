...
func main() {
	log.Println("Starting make-action-pose process")
	...
	// Upload mask image
	maskImagePath := filepath.Join(config.ImagePath, "pose_mask_0.png")
	maskImageName, err := utils.SimpleUploadFileToS3(maskImagePath)
	if err != nil {
		log.Fatalf("Failed to upload mask image: %v", err)
	}
	log.Printf("Mask image uploaded successfully, image name: %s", maskImageName)
	
	// Execute make-action-pose
	requestModel := models.MakeActionPose{
	...
	response, err := utils.MakeAuthenticatedRequest("POST", "/api/v1/make-action-pose", requestModel, nil)
	if err != nil {
		log.Fatalf("Error making make-action-pose request: %v", err)
	}
	...
	if promptResponse.Code != 0 {
		log.Fatalf("Make-action-pose execution failed, error message: %s", promptResponse.Msg)
	}
	...
	// Get history
	getHistoryReq := models.GetHistory{ToolID: "5"} // Assuming tool_id for make-action-pose is 5
