...
func main() {
	log.Println("Starting outpainting process")
	...
	// Execute outpainting
	requestModel := models.Outpainting{
		Description: "street",
		ImageName:   imageName,
		Left:        0,
		Top:         0,
	...
	response, err := utils.MakeAuthenticatedRequest("POST", "/api/v1/outpainting", requestModel, nil)
	if err != nil {
		log.Fatalf("Error making outpainting request: %v", err)
	}
