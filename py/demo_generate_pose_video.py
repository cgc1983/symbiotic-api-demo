...
from models import GeneratePoseVideo, PromptResponse, GetTaskStatus, StatusResponse, GetHistory, HistoryResponse
...
def main():
    ...
    request_model = GeneratePoseVideo(
    ...
    response = make_authenticated_request(
        "POST", "/api/v1/generate-pose-video", 
        data=request_model.model_dump()
    )
    prompt_model = PromptResponse(**response.json())
    if prompt_model.code != 0:
        logger.error(f"Generate-pose-video execution failed, error message: {prompt_model.msg}")
        return
    ...
    # Get history
    get_history_req = GetHistory(tool_id="6")  # Assuming tool_id for generate-pose-video is 6
    response = make_authenticated_request(
