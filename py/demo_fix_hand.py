...
from models import FixHand, PromptResponse, GetTaskStatus, StatusResponse, GetHistory, HistoryResponse
...
def main():
    ...
    if image_name:
        logger.info(f"Character image uploaded successfully, image name: {image_name}")
    else:
        logger.error("Failed to upload character image")
        return

    # Execute fix-hand
    request_model = FixHand(
        character_image_name=image_name,
        lora_type="1",  # Disney style
        prompt_text="Fix the hand position and make it look natural",
        pose_type="0"   # Idle left-facing
    )
    
    response = make_authenticated_request(
        "POST", "/api/v1/fix-hand", 
        data=request_model.model_dump()
    )
    
    prompt_model = PromptResponse(**response.json())
    if prompt_model.code != 0:
        logger.error(f"Fix-hand execution failed, error message: {prompt_model.msg}")
        return
    ...
    # Get history
    get_history_req = GetHistory(tool_id="5")  # Assuming tool_id for fix-hand is 5
    response = make_authenticated_request(
