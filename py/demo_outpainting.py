...
from models import Outpainting, PromptResponse, GetTaskStatus, StatusResponse, GetHistory, HistoryResponse
...
def main():
    ...
    response = make_authenticated_request(
        "POST", "/api/v1/outpainting", 
        data=request_model.model_dump()
    )
    prompt_model = PromptResponse(**response.json())
    if prompt_model.code != 0:
        logger.error(f"Outpainting execution failed, error message: {prompt_model.msg}")
        return
